<template>
    <div>
        <!-- Navigation -->
        <nav class="navbar navbar-expand-lg navbar-light top">
            <div class="container">
                <a class="navbar-brand" href="#">Fetch</a>
                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarResponsive" aria-controls="navbarResponsive" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span> 
                </button>
                <div class="collapse navbar-collapse" id="navbarResponsive">
                    <ul class="navbar-nav ml-auto">
                        
                        <li class="nav-item avatar dropdown">
                            <div class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-5" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                <span class="badge badge-danger ml-2">1</span>
                                <i class="fas fa-envelope"></i>
                            </div>
                            <div class="dropdown-menu dropdown-menu-lg-right dropdown-secondary" aria-labelledby="navbarDropdownMenuLink-5">
                                <a class="dropdown-item" href="customer_orders.html">Order delivered </a>
                            </div>
                        </li>
                        
                        <li class="nav-item">
                            <a class="nav-link" href="customer_orders.html">Orders</a>
                        </li>
                        
                        <li class="nav-item">
                            <a class="nav-link" href="customer_cart.html">Cart</a>
                        </li>
                        
                        <li>
                            <a class="nav-link" href="admin.html">Admin Dashboard</a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>

        <!-- Full Page Image Header with Vertically Centered Content -->
        <header class="masthead">
            <div class="container h-100">
                <div class="row h-100 align-items-center">
                    <div class="col-12 text-center">
                        <h1 class="font-weight-light">Fetch</h1>
                        <p class="lead">
                            <a href="#content-1" class="no-style-link">Order now</a>
                        </p>
                    </div>
                </div>
            </div>
        </header>

        <!-- Page Content -->

        <section class="py-5 page-content-1" id="content-1">
            <!-- Restaurants Info -->
            <div class="container">
                <h1 class="font-weight-light">Restaurants Near You</h1>
            </div>
            <div class="container">
                <div class="row">
                    <div class="col-sm">
                        <a class="no-style-link" href="restaurant1.html">
                            <img class="food-image" src="img/burger.jpg" alt="Generic placeholder image">
                            <br>
                            <div class="restaurant-label">
                                <text>Restaurant 1</text>
                                <br>
                                <text class="food-franchise-delivery">Delivery: $5</text>
                            </div>
                        </a>
                    </div>
                    <div class="col-sm">
                        <a class="no-style-link" href="restaurant1.html">
                            <img class="food-image" src="img/sushi.jpg" alt="Generic placeholder image">
                            <br>
                            <div class="restaurant-label">
                                <text class="food-franchise">Restaurant 2</text>
                                <br>
                                <text class="food-franchise-delivery">Delivery: $5</text>
                            </div>
                        </a>
                    </div>
                    <div class="col-sm">
                        <a class="no-style-link" href="restaurant1.html">
                            <img class="food-image" src="img/pizza.jpg" alt="Generic placeholder image">
                            <br>
                            <div class="restaurant-label">
                                <text class="food-franchise">Restaurant 3</text>
                                <br>
                                <text class="food-franchise-delivery">Delivery: $5</text>
                            </div>
                        </a>
                    </div>
                </div>
            </div>
            <!-- End Restaurants Info -->
        </section>

        <!-- Section 2 Content-->
        <section class = "py-5 page-content-2">
            <div class="container">
                <div class="col-sm">
                    <text>More advertising space here</text>
                </div>
                <div class="col-sm">
                    <img src="https://via.placeholder.com/800x500">
                </div>
            </div>
        </section>
        <!-- Section 2 Content End -->

        <!-- Footer -->
        <section class = "py-5 page-content-1">
            <div class="container">
                <div class="col-sm">
                    <text>Contact us, etc. here</text>
                </div>
            </div>
        </section>
        <!-- Footer End -->
    </div>
</template>
